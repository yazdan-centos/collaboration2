...
function timestamp(value) {
  return Number.isNaN(new Date(value || 0).getTime()) ? 0 : new Date(value).getTime();
}
...
function TicketCreateForm({ auth, isManager, customerContracts, onSubmit, onCancel }) {
  const authenticatedCustomerId = auth?.customerId ?? auth?.userId;
  const [values, setValues] = useState({ title: '', description: '', customerId: isManager ? '' : String(authenticatedCustomerId ?? ''), slaContractId: '', assignedMemberId: '' });
  ...
  async function uploadFiles(ticketId, fileList) {
    const results = await Promise.allSettled(fileList.map((file) => ticketService.uploadCustomerAttachment(ticketId, file)));
    const failures = [];
    results.forEach((result, index) => { if (result.status === 'rejected') failures.push(fileList[index]); });
  ...
  async function handleSubmit(event) {
    ...
    try {
      const payload = {
        ...
        assignedMemberId: isManager && values.assignedMemberId ? Number(values.assignedMemberId) : null,
      };
    ...
    <form onSubmit={handleSubmit}>
      <div className="client-form-grid">
        ...
        <label className="sla-form-field client-form-wide">
          <span>عنوان</span>
          <input name="title" value={values.title} onChange={updateField} required disabled={isSubmitting} />
          {fieldErrors.title && <span className="client-field-error">{fieldErrors.title}</span>}
        </label>
        <label className="sla-form-field client-form-wide">
          <span>شرح مشکل</span>
          <textarea name="description" rows="5" value={values.description} onChange={updateField} required disabled={isSubmitting} />
          {fieldErrors.description && <span className="client-field-error">{fieldErrors.description}</span>}
        </label>
        {!isManager && (
          <label className="sla-form-field client-form-wide">
            <span>قرارداد SLA (اختیاری)</span>
            <select name="slaContractId" value={values.slaContractId} onChange={updateField} disabled={isSubmitting}>
              <option value="">بدون قرارداد</option>
              {customerContracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.contractName || contract.name || `قرارداد ${contract.id}`}</option>)}
            </select>
          </label>
        )}
        {isManager && (
          <>
            <label className="sla-form-field">
              ...
              <input name="customerId" type="number" min="1" value={values.customerId} onChange={updateField} required disabled={isSubmitting} />
              {fieldErrors.customerId && <span className="client-field-error">{fieldErrors.customerId}</span>}
            </label>
            <label className="sla-form-field">
              <span>شناسه قرارداد SLA (اختیاری)</span>
              <input name="slaContractId" type="number" min="1" value={values.slaContractId} onChange={updateField} disabled={isSubmitting} />
            </label>
            <label className="sla-form-field">
              <span>شناسه کارشناس (اختیاری)</span>
              <input name="assignedMemberId" type="number" min="1" value={values.assignedMemberId} onChange={updateField} disabled={isSubmitting} />
            </label>
          </>
        )}
        <label className="sla-form-field client-form-wide">
          ...
          {files.length > 0 && <span className="client-field-error">{files.length} فایل انتخاب شده</span>}
        </label>
      </div>
      <div className="sla-edit-actions">
        <button type="button" className="filter-btn" onClick={onCancel} disabled={isSubmitting}>انصراف</button>
        <button type="submit" className="primary-action-btn" disabled={isSubmitting}>
          <i className={isSubmitting ? 'fas fa-spinner fa-spin' : 'fas fa-paper-plane'} />
...
function TicketEditForm({ ticket, onSaved, onCancel }) {
  ...
  const isExpert = isAssignedExpert(auth, ticket);
  const canEditTitle = isManager;
  const canEditStatus = isManager || isExpert;
  const [values, setValues] = useState({ title: ticket.title || '', description: ticket.description || '', status: ticket.status || '', statusNote: '' });
  const [fieldErrors, setFieldErrors] = useState({});
  const [formError, setFormError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    setValues({ title: ticket.title || '', description: ticket.description || '', status: ticket.status || '', statusNote: '' });
  }, [ticket]);

  const availableStatuses = useMemo(() => NEXT_TICKET_STATUSES[ticket.status] || [], [ticket.status]);
  ...
  async function handleSubmit(event) {
    ...
    if (canEditStatus) {
      ...
      if (values.statusNote.trim()) payload.statusNote = values.statusNote.trim();
    }
          ...
          <>
            <label className="sla-form-field">
              ...
              <select name="status" value={values.status} onChange={updateField} disabled={isSubmitting}>
                <option value={ticket.status}>{statusMeta[ticket.status]?.label || ticket.status}</option>
                {availableStatuses.map((status) => <option key={status} value={status}>{statusMeta[status]?.label || status}</option>)}
              </select>
              {fieldErrors.status && <span className="client-field-error">{fieldErrors.status}</span>}
            </label>
            <label className="sla-form-field client-form-wide">
              <span>یادداشت تغییر وضعیت</span>
              <textarea name="statusNote" rows="3" value={values.statusNote} onChange={updateField} disabled={isSubmitting} />
            </label>
          </>
        )}
        {!canEditTitle && !canEditStatus && <div className="sla-form-alert" role="alert">شما اجازه ویرایش این تیکت را ندارید.</div>}
      </div>
      <div className="sla-edit-actions">
        <button type="button" className="filter-btn" onClick={onCancel} disabled={isSubmitting}>انصراف</button>
        {(canEditTitle || canEditStatus) && (
          <button type="submit" className="primary-action-btn" disabled={isSubmitting}>
            <i className={isSubmitting ? 'fas fa-spinner fa-spin' : 'fas fa-floppy-disk'} />
...
function TicketView({ ticket, ticketId, onUpdated }) {
  ...
  const attachments = useMemo(() => [...(ticket?.attachments || [])].sort((a, b) => timestamp(a.uploadedAt || a.createdAt) - timestamp(b.uploadedAt || b.createdAt)), [ticket]);
  ...
  const attachmentApi = useMemo(() => ({
    ...
    ...(isManager ? { delete: async (file) => ticketService.deleteAttachment(file.serverId || file.id) } : {}),
    download: async (file) => resolveAttachmentUrl(file),
    share: async (file) => resolveAttachmentUrl(file),
  }), [attachments, isCustomer, isManager, ticketId]);
  ...
  return (
    <div className="ticket-modal-view">
      <div className="ticket-modal-meta">
        ...
        <span className={`ticket-status ${status.className}`}><span className="dot" />{status.label}</span>
      </div>
      <h2 className="ticket-modal-title">{ticket.title || 'بدون عنوان'}</h2>
      {ticket.description && <p className="ticket-modal-description">{ticket.description}</p>}
      <div className="ticket-modal-dates fa-num">
        <span><i className="fas fa-calendar" /> ایجاد: {formatDate(ticket.createdAt)}</span>
        <span><i className="fas fa-rotate" /> بروزرسانی: {formatDate(ticket.updatedAt)}</span>
      </div>
      <section className="ticket-modal-section">
        <EmbeddedChat ticketId={ticketId} canSend={canSendMessage} height="320px" onMessageCreated={() => onUpdated()} />
      </section>
      {canUpload && (
        <section className="ticket-modal-section">
          <FileUploadManager
...
export default function TicketModal({ mode, ticketId, ticketSummary, onClose, onSuccess }) {
  const { auth } = useAuth();
  const isManager = hasRole(auth, USER_ROLES.TEAM_MANAGER);
  const isCustomer = hasRole(auth, USER_ROLES.CUSTOMER);
  ...
  useEffect(() => {
    if (mode === 'create') {
      ...
      if (isCustomer && customerId) {
        ticketService.getCustomerById(customerId).then((customer) => {
          const contracts = customer?.slaContracts || customer?.slaContractIds || [];
    ...
    async function load() {
      ...
      try {
        ...
        if (err?.code !== 'ERR_CANCELED') setError(apiErrorMessage(err, 'دریافت جزئیات تیکت با خطا مواجه شد.'));
      } finally {
  ...
  const title = mode === 'create' ? 'ثبت تیکت جدید' : mode === 'edit' ? 'ویرایش تیکت' : 'جزئیات تیکت';

  return (
    <div className="client-form-backdrop" role="presentation">
      <div className="panel client-form-panel ticket-modal-panel" role="dialog" aria-modal="true" aria-labelledby="ticket-modal-title">
        <div className="panel-header client-form-header">
          <div>
            <div className="panel-title" id="ticket-modal-title">{title}</div>
            <div className="team-results-count">{mode === 'create' ? 'درخواست خود را ثبت کنید' : 'اطلاعات تیکت را مشاهده و پیام ارسال کنید'}</div>
          </div>
          <button type="button" className="icon-action-btn" onClick={onClose} aria-label="بستن">
            <i className="fas fa-xmark" />
          </button>
        </div>
        <div className="ticket-modal-body">
          {isLoading ? (
            <div className="team-empty compact"><i className="fas fa-spinner fa-spin" /><span>در حال دریافت...</span></div>
          ) : error ? (
            <div className="team-empty compact" role="alert"><span>{error}</span><button type="button" className="filter-btn" onClick={() => setReloadKey((k) => k + 1)}>تلاش دوباره</button></div>
          ) : mode === 'create' ? (
            <TicketCreateForm auth={auth} isManager={isManager} customerContracts={customerContracts} onSubmit={(created) => { if (onSuccess) onSuccess(created); onClose(); }} onCancel={onClose} />
          ) : mode === 'edit' ? (
            <TicketEditForm ticket={ticket} onSaved={handleSaved} onCancel={onClose} />
          ) : (
            <TicketView ticket={ticket} ticketId={ticketId} onUpdated={() => setReloadKey((k) => k + 1)} />
          )}
        </div>
      </div>
    </div>
