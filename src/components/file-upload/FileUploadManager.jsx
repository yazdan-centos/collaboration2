...
export const FILE_UPLOAD_STYLES = {
  container:
      'rounded-2xl border border-slate-700 bg-slate-900 p-5 text-slate-100 shadow-xl',
  ...
  dropzone:
      'flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-600 bg-slate-950/40 p-6 text-center transition hover:border-emerald-500 hover:bg-emerald-500/5',
  ...
  fileIcon:
      'flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-800 text-lg text-emerald-400',
  ...
  button:
      'inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-500 px-4 py-2 text-xs font-bold text-white transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-50',
  secondaryButton:
      'inline-flex items-center justify-center gap-2 rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-xs text-slate-200 transition hover:border-emerald-500 hover:text-emerald-400 disabled:cursor-not-allowed disabled:opacity-50',
  iconButton:
      'inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-700 text-slate-400 transition hover:border-emerald-500 hover:text-emerald-400 disabled:opacity-50',
  dangerButton:
      'inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-700 text-slate-400 transition hover:border-red-500 hover:text-red-400 disabled:opacity-50',
...
function validateFile(file, acceptedExtensions, maxFileSize) {
  if (file.size === 0) {
    return 'فایل خالی است و قابل ارسال نیست.';
  }
...
export function PreflightFileList({
                                    files,
                                    styles,
                                    onRemove,
                                    onUpload,
  ...
  return (
      <section className={styles.section}>
        <header className={styles.sectionHeader}>
          <div>
            <strong className="text-sm">فایل‌های آماده ارسال</strong>
            <span className="mr-2 text-xs text-slate-500">{files.length} فایل</span>
          </div>
          ...
          <button
              ...
              onClick={onUpload}
              disabled={!readyCount || isUploading}
          >
            <i className="fas fa-cloud-arrow-up" /> شروع آپلود
          </button>
        </header>
        ...
        {files.length ? (
            ...
            <div className={styles.emptyState}>هنوز فایلی انتخاب نشده است.</div>
        )}
      </section>
  );
}
...
export function UploadedFilesList({
  ...
  return (
      <section className={styles.section}>
        <header className={styles.sectionHeader}>
          <strong className="text-sm">فایل‌های آپلودشده</strong>
          <span className="text-xs text-slate-500">{files.length} فایل</span>
        </header>
...
export default function FileUploadManager({
  ...
  const styles = useMemo(
      () => ({ ...FILE_UPLOAD_STYLES, ...styleOverrides }),
      [styleOverrides]
  );
  ...
  useEffect(() => {
    ...
    async function loadFiles() {
      ...
      try {
        ...
        if (active) {
          setUploadedFiles(
              (Array.isArray(response) ? response : response?.content || []).map(
        ...
        if (active) {
          setListError('دریافت فایل‌های آپلودشده انجام نشد.');
        }
      } finally {
        if (active) {
          setIsLoading(false);
      ...
      return [
        ...
        ...incoming.map((file, index) => {
          ...
          const error = duplicate
              ...
              : countExceeded
                  ? `حداکثر ${maxFiles} فایل قابل انتخاب است.`
                  : validateFile(
                      file,
          ...
          return {
            ...
            type: file.type,
            progress: 0,
            status: error ? 'validation-error' : 'ready',
            error,
            uploadedUrl: '',
          };
        }),
  ...
  async function uploadOne(item) {
    setFiles((current) =>
        current.map((file) =>
            file.id === item.id
                ? { ...file, status: 'uploading', progress: 0, error: '' }
                : file
        )
    );
          ...
          current.map((file) =>
              file.id === item.id
                  ? {
                    ...file,
                    status: 'error',
                    error: getApiErrorMessage(error, 'آپلود فایل انجام نشد.'),
                  }
                  : file
          )
      );
  ...
  async function uploadAll() {
    if (!api?.upload) {
      setListError('تابع API آپلود تعریف نشده است.');
      return;
    }
  ...
  async function resolveDownload(file) {
    ...
    try {
      ...
      if (!url) {
        throw new Error('آدرس دانلود موجود نیست.');
      }
      ...
      setListError(error.message || 'دانلود فایل انجام نشد.');
    } finally {
      setBusyAction('');
    }
  }
  ...
  async function shareFile(file) {
    ...
    try {
      ...
      if (!url) {
        throw new Error('لینک اشتراک موجود نیست.');
      }
      ...
      if (error.name !== 'AbortError') {
        setListError(error.message || 'اشتراک فایل انجام نشد.');
      }
    } finally {
      setBusyAction('');
    }
  ...
  async function deleteFile(file) {
    if (!window.confirm(`فایل «${file.name}» حذف شود؟`)) {
      return;
    }
    ...
    try {
      ...
      setListError(getApiErrorMessage(error, 'حذف فایل انجام نشد.'));
    } finally {
      setBusyAction('');
    }
  }
  ...
  return (
      <div className={styles.container}>
        ...
        >
          ...
          یا برای انتخاب چند فایل کلیک کنید
        </span>
          <span className="mt-1 text-[10px] text-slate-500">
          حداکثر {maxFiles} فایل، هر فایل تا {formatSize(maxFileSize)}
        </span>
        </div>
        ...
        <UploadedFilesList
            files={uploadedFiles}
            styles={styles}
            onDownload={resolveDownload}
            onShare={shareFile}
            onDelete={api?.delete ? deleteFile : undefined}
            busyAction={busyAction}
            isLoading={isLoading}
            error={listError}
        />
