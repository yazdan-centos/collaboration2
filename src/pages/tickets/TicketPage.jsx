...
function renderStatus(ticket) {
    ...
    return <span
        className={`ticket-status ${status.className}`}>
        <span className="dot"/>
        {status.label}
    </span>;
}
...
function TicketRow({ticket, isSelected, onSelect, showTriage, showManagement, onDelete, canDelete}) {
    const status = getStatusMeta(ticket.status);
            ...
            {showTriage && <td>
                {priority ? (
                    <span className={`priority-badge ${priority.className}`}>
                        <i className={priority.icon} aria-hidden="true"/>
                        {priority.label}
                    </span>
                ) : <span className="ticket-muted">—</span>}
            </td>}
            {showManagement && <>
                <td>{getCustomerFullName(getCustomerById(ticket.customerId)) || (ticket.customerId ? `مشتری ${ticket.customerId}` : '—')}</td>
...
export default function TicketPage() {
    ...
    useEffect(() => {
        const timer = window.setTimeout(() => setDebouncedFullNameQuery(fullNameQuery.trim()), 350);
        return () => window.clearTimeout(timer);
    }, [fullNameQuery]);

    useEffect(() => {
        setPage(0);
    }, [debouncedQuery, debouncedFullNameQuery, statusFilter]);
    ...
    useEffect(() => {
        ...
        async function loadTickets() {
            ...
            if (debouncedFullNameQuery) filters.fullName = debouncedFullNameQuery;
    ...
    }, [debouncedQuery, debouncedFullNameQuery, statusFilter, page, reloadKey]);

    function clearFilters() {
        setQuery('');
        setDebouncedQuery('');
        setFullNameQuery('');
        setDebouncedFullNameQuery('');
        setStatusFilter('all');
        setPage(0);
    }
                ...
                <div className="panel ticket-directory animate-in delay-1">
                    <div className="panel-header ticket-directory-header">
                        ...
                        <div className="panel-actions ticket-toolbar">
                            {canCreate && <button type="button" className="filter-btn active"
                                                                  onClick={() => navigate('/tickets/new')}><i
                                className="fas fa-plus"/> ایجاد تیکت</button>}
                            ...
                            <label className="team-search">
                                ...
                                <input
                                    type="search"
                                    value={fullNameQuery}
                                    onChange={(event) => setFullNameQuery(event.target.value)}
                                    placeholder="جستجو در نام و نام خانوادگی..."
                                    aria-label="جستجوی تیکت‌ها بر اساس نام و نام خانوادگی"
                                />
                            </label>
                            ...
                            <button type="button" className="filter-btn"
                                    onClick={() => setReloadKey((value) => value + 1)} disabled={isLoading}>
                                <i className={isLoading ? 'fas fa-spinner fa-spin' : 'fas fa-rotate'}
                                   aria-hidden="true"/>
                                به‌روزرسانی
                            </button>
                        </div>
                    ...
                    {isLoading && !ticketPage.content.length ? (
                        ...
                        <div className="team-empty" role="alert">
                            ...
                            <span>{loadError}</span>
                            <button type="button" className="filter-btn"
                                    onClick={() => setReloadKey((value) => value + 1)}>تلاش دوباره
                            </button>
                        </div>
                        ...
                        <>
                            ...
                            <div className="ticket-pagination">
                                <span
                                    className="fa-num">نمایش {visibleStart} تا {visibleEnd} از {ticketPage.totalElements}</span>
                                <div>
                                    <button type="button" className="filter-btn"
                                            disabled={ticketPage.first || isLoading}
                                            onClick={() => setPage((value) => Math.max(0, value - 1))}>
                                        <i className="fas fa-chevron-right"/> قبلی
                                    </button>
                                    <span
                                        className="fa-num">صفحه {ticketPage.number + 1} از {Math.max(ticketPage.totalPages, 1)}</span>
                                    <button type="button" className="filter-btn" disabled={ticketPage.last || isLoading}
                        ...
                        <div className="team-empty">
                            ...
                            {(debouncedQuery || debouncedFullNameQuery || statusFilter !== 'all') &&
                                <button type="button" className="filter-btn" onClick={clearFilters}>پاک کردن
                                    فیلترها</button>}
                        </div>
                    )}
                </div>
