    ...
    <section className="sla-page">
      <form className="panel sla-edit-panel animate-in" onSubmit={handleSubmit}>
        ...
        <div className="panel-header sla-edit-header">
          <div><span className="sla-edit-kicker"><i className="fas fa-wand-magic-sparkles" /> فضای ویرایش قرارداد</span><div className="panel-title">ویرایش قرارداد SLA</div><div className="team-results-count fa-num">شناسه قرارداد #{contractId}</div></div>
        ...
        <div className="sla-edit-form">
          ...
          <label className="sla-form-field"><span><i className="fas fa-file-signature" /> نام قرارداد</span><input name="contractName" value={form.contractName} onChange={updateField} required /></label>
          <label className="sla-form-field"><span><i className="fas fa-building" /> شناسه مشتری</span><input name="customerId" type="number" min="1" value={form.customerId} onChange={updateField} /></label>
          <label className="sla-form-field"><span><i className="fas fa-bolt" /> زمان پاسخ (ساعت)</span><input name="responseTimeHours" type="number" min="0" step="0.5" value={form.responseTimeHours} onChange={updateField} /></label>
          <label className="sla-form-field"><span><i className="fas fa-screwdriver-wrench" /> زمان رفع مشکل (ساعت)</span><input name="resolutionTimeHours" type="number" min="0" step="0.5" value={form.resolutionTimeHours} onChange={updateField} /></label>
          <label className="sla-form-field"><span><i className="fas fa-chart-line" /> آپ‌تایم هدف (%)</span><input name="targetUptime" type="number" min="0" max="100" step="0.01" value={form.targetUptime} onChange={updateField} /></label>
          <label className="sla-form-field"><span><i className="fas fa-headset" /> ساعات پشتیبانی</span><input name="supportCoverage" value={form.supportCoverage} onChange={updateField} placeholder="مثال: 24/7" /></label>
          <label className="sla-form-field"><span><i className="fas fa-toggle-on" /> وضعیت</span><select name="status" value={form.status} onChange={updateField}><option value="ACTIVE">فعال</option><option value="INACTIVE">غیرفعال</option><option value="EXPIRED">منقضی</option><option value="CANCELLED">لغو شده</option></select></label>
          <label className="sla-form-field sla-form-wide"><span><i className="fas fa-layer-group" /> محدوده خدمات</span><textarea name="serviceScope" rows="4" value={form.serviceScope} onChange={updateField} /></label>
          <label className="sla-form-field sla-form-wide"><span><i className="fas fa-note-sticky" /> یادداشت‌ها</span><textarea name="notes" rows="4" value={form.notes} onChange={updateField} /></label>
        </div>
